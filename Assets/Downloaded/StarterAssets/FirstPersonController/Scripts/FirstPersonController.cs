	...
	public class FirstPersonController : MonoBehaviour
	{
		...
		private void PlayFootStepAudio()
		{
			...
			if (Grounded && _speed > 0.0f && Time.time > _nextStep)
			{
				...
				_audioSourceStep.clip = footstepSounds[n];
				_audioSourceStep.PlayOneShot(_audioSourceStep.clip);
				// move picked sound to index 0 so it's not picked next time
				footstepSounds[n] = footstepSounds[0];
				footstepSounds[0] = _audioSourceStep.clip;
			}
		}

		private bool jumpSoundTemp = true;
		private bool LandSoundTemp = false;

		private void PlayJumpSound()
        {

            if (Grounded && jumpSoundTemp)
            {
